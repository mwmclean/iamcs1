...
```{r docallEx, eval = c(1, 2, 4), tidy = FALSE}
...
```{r Frames1, tidy = FALSE, cache = TRUE}
...
```{r Frames2, tidy = FALSE, dependson = "Frame1", cache = TRUE}
env$f(100, env)  # assign 100 to x in env
```
```{r eval = FALSE, dependson = "Frames2"}
get("x", env)  # retrieve value of x in env              ## 100
```
```{r eval = FALSE, dependson = "Frames2", eval = 1}
env$f(10, parent.env(env))  # assign 10 to x in env's parent environment: R_GlobalEnv
```
```{r eval = FALSE, dependson = "Frames2", tidy = FALSE}
x                                                        ## 10
```
```{r tidy = FALSE, cache = FALSE, eval = FALSE}
...
- source code can be viewed by typing the function name at the console
  ...
  - sometimes backticks (`r "\x60"`) are necessary

  ```
  `::`
  ```
  ```{r dblcolon, echo = FALSE}
  eval(parse(text="\x60::\x60"))
  ```
...
```{r tidy = FALSE, cache = FALSE, eval = TRUE}
...
```{r eval = TRUE, highlight = FALSE, tidy = FALSE, echo = -1, cache = FALSE}
options(width = 120)
...
```{r eval = TRUE, highlight = TRUE, tidy = TRUE, cache = FALSE, echo = -1}
options(width = 100)
